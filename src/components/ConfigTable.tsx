...
export default function ConfigTable({
  ...
  const [configs, setConfigs] = useState<EmailConfig[]>(initialConfigs);
  ...
  const [sortColumn, setSortColumn] = useState<
    "profileName" | "userName" | "mailFolder" | null
  >(null);
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("asc");
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [configToDelete, setConfigToDelete] = useState<EmailConfig | null>(null);

  const { isWizardOpen, setIsWizardOpen } = useStore();

  const handleSort = (column: "profileName" | "userName" | "mailFolder") => {
  ...
  const confirmDelete = () => {
    if (configToDelete) {
      setConfigs(configs.filter((c) => c.id.toString() !== configToDelete.id.toString()));
  ...
  return (
    <>
      <div className="flex gap-0 h-full overflow-hidden bg-muted/30">
        <div
          className={`flex flex-col transition-all duration-300 ${
            isWizardOpen ? "w-2/3" : "w-full"
          }`}
        >
          {/* Header */}
                  ...
                  {sortedConfigs.length === 0 ? (
                    ...
                    sortedConfigs.map((config) => (
                      <TableRow key={config.id} className="group">
                        ...
                        <TableCell className="font-regular">{config.profileName}</TableCell>
                        <TableCell className="text-primary">{config.userName}</TableCell>
                        <TableCell className="text-muted-foreground">{config.mailFolder}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete &quot;{configToDelete?.profileName}&quot;? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
