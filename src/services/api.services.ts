...
import { ApiResponse, Email, EmailConfig, EmailUser, MailFolder, Rule, RuleData } from "@/types/config";

const getApiInstance = () => {
  const { configs } = useStore.getState();
  return axios.create({
    baseURL: (configs as any)?.NEXT_PUBLIC_API_BASEPATH_URL || "",
...
const getApi2Instance = () => {
  const { configs } = useStore.getState();
  return axios.create({
    baseURL: (configs as any)?.NEXT_PUBLIC_API_2_BASEPATH_URL || "",
    withCredentials: false,
    headers: { "Content-Type": "application/json" },
  });
};
...
export const getAllEmails = async (): Promise<Email[]> => {
  try {
    ...
    const endPoint = (configs as any)?.GET_ALL_EMAILS_END_POINT || "Email/All";
...
export const convertEmailToConfig = (email: Email): EmailConfig => {
  const rules: Rule[] = email.mailRule.map((mailRule, index) => {
    let ruleData: RuleData | null = null;
...
export const convertConfigToEmail = (config: EmailConfig): Partial<Email> => {
...
// search email users 
export const fetchEmailUsers = async (value: string): Promise<EmailUser[]> => {
  try {
    const { configs } = useStore.getState();
    const api = getApiInstance();
    const endPoint = (configs as any)?.SEARCH_EMAIL_USERS_END_POINT || "User/Users?filter=";
...
// search mail folders
export const fetchMailFolders = async (emailUser: string): Promise<MailFolder[]> => {
  try {
    const { configs } = useStore.getState();
    const api = getApiInstance();
    const endPoint = (configs as any)?.GET_USERS_FOLDER_END_POINT || "User/UserFolders?email=";
...
export const fetchClientsList = async (
  ...
  try {
    ...
    const endPoint =
      (configs as any)?.GET_CLIENTS_LIST_END_POINT ||
